...
const Sidebar = () => {
  ...
  const handleLogout = () => {
    // Here you can also clear auth tokens if needed
      ...
      >
        ...
        <button
          ...
          className={`
            ...
            px-5 py-3 rounded-[12px] transition-all
            hover:bg-[#1A2340]
            ${isOpen ? "w-[90%]" : "w-[60px] justify-center"}
          `}
        >
          <FiLogOut className="md:w-[22px] md:h-[22px] w-[18px] h-[18px]" />
          {isOpen && <span className="text-[16px]">Logout</span>}
        </button>
      </div>

      {/* CONTENT SPACING */}
      <div className={`${isOpen ? "ml-[20%]" : "ml-[80px]"} transition-all duration-300`}></div>
    </>
  );
};
