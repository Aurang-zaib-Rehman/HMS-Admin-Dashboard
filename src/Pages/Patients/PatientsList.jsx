...
//                     <Link to={`/patients/edit/${p.id}`} className="px-2 py-1 bg-gray-200 text-gray-800 rounded text-xs">Edit</Link>
...
const PatientsList = () => {
    ...
    () => [
      {
        ...
        value: 1500,
        icon: <FaUserInjured size={34} className="text-blue-600" />,
        bg: "bg-blue-100",
      },
      {
        title: "Total Surgeries",
        value: 150,
        icon: <FaProcedures size={34} className="text-red-600" />,
        bg: "bg-red-100",
      },
      {
        title: "Currently Admitted",
        value: patients.filter((p) => p.status === "Admitted").length,
        icon: <FaHospitalUser size={34} className="text-green-600" />,
        bg: "bg-green-100",
      },
    ],
    [patients]
  ...
  return (
    <div className="w-full p-4 sm:p-6 max-w-full overflow-x-hidden">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-6">
        {stats.map((card, i) => (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            className={`flex items-center gap-4 p-5 rounded-xl shadow border ${card.bg}`}
          >
            {card.icon}
            <div>
      ...
      {/* Desktop Table */}
      <div className="hidden sm:block bg-white rounded-xl shadow border p-5">
        <h2 className="text-lg font-semibold mb-3">Recent Patients</h2>

        <table className="w-full text-left">
          <thead className="bg-gray-100">
            <tr>
              {["ID", "Name", "Age", "Gender", "Disease", "Operation", "Status", "Actions"].map(
                (h) => (
                  <th key={h} className="p-3 text-sm font-semibold">
                    {h}
                  </th>
                )
              )}
                ...
                >
                  ...
                  <td className="p-3 flex gap-2">
                    ...
                    <button
                      ...
                      className="px-2 py-1 bg-red-200 text-red-700 rounded text-xs"
                    >
                      Delete
                    </button>
                  </td>
      ...
      {/* Mobile List (compact, NOT cards) */}
      <div className="sm:hidden divide-y bg-white rounded-xl shadow border">
        <AnimatePresence>
          {displayedPatients.map((p) => (
            <motion.div
              key={p.id}
